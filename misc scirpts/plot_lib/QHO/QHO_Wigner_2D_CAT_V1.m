...
psi0 = (1/(sqrt(2)))*(psi0+Anorm*exp(-aa*(xv+xbar).^2) .* exp(1i*k0*(xv)));
...
for j = 1:nf
    ...
    title('$\mathrm{Wigner-function}, \ U(x)=\frac{1}{2}\kappa x^2, \ \kappa=10$','fontsize',18,'Interpreter','latex')
...
for j = 1:nf
    ...
    contour3( xtemp, ktemp, 100+Htest,0:20:300,'-b'   )
    hold on
...
%%
video=VideoWriter('QHO_Wigner_2D_CAT_V1','MPEG-4');
video.Quality=100;
video.FrameRate=20;
open(video);
writeVideo(video,F2);
...
%%
video=VideoWriter('QHO_psi_phi_CAT_V1');
video.Quality=100;
video.FrameRate=20;
open(video);
writeVideo(video,F);
